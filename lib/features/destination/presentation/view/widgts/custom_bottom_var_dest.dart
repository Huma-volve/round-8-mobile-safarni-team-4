...
class CusotmBottomBarDest extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // نستخدم BlocBuilder لعرض السعر الحقيقي
    return BlocBuilder<DestinationCubit, DestinationState>(
      builder: (context, state) {
        // نعرّف متغيّر للسعر، ونضع قيمة افتراضية في حالة التحميل
        String price = "0.00";

        if (state is DestinationSuccess) {
          price = state.tour.price; // السعر القادم من الـ API
        }
        ...
        return Container(
          ...
          child: Row(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                        "\$$price", // السعر الديناميكي
                        style: AppThemes.light.textTheme.titleMedium!.copyWith(
                            color: AppColors.mainColorLight[70],
                            fontWeight: FontWeight.w700, // جعلته أثقل ليكون واضحاً
                            fontSize: 18.sp),
                      ),
                      Text(
                        " /person",
              ...
              ElevatedButton(
                onPressed: state is DestinationSuccess ? () {
                  // ننتقل للدفع ونمرر البيانات لو احتجتِ
                  context.pushNamed(AppRoutes.paymentMethod);
                } : null, // الزر يكون معطل حتى يتم تحميل البيانات
